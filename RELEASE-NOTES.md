# Koollook 2.7.0 (tester)

Plasma 6 only. Download `koollook-2.7.0.tar.zst`, extract, run `./install.sh` and pick pieces (or use flags). No root.
...
| File | What |
|------|------|
| `koollook-2.7.0.tar.zst` | Full pack + chooser `install.sh` |
| `koollook-widgets-2.7.0.tar.zst` | All plasmoids |
| `koollook-theme-2.7.0.tar.zst` | Colors, icons, Koollook + Koollook Dotted |
| `koollook-accessibility-2.7.0.tar.zst` | STT helper |
| `com.koollook.*.plasmoid` | Single widgets (Store-style) |

Widgets: Planisphere, Calendar, Weather, STT Clip, STT tray, Wavebar.
